...
base_dir = '/Users/hulanfang/长期助理/hu_project'
images_dir = os.path.join(base_dir, 'assets/reading/images')
audio_dir = os.path.join(base_dir, 'assets/reading/audio')

print('--- Resource Check Report ---')

for book_id, config in folder_map.items():
    # Check first page image
    first_page_path = os.path.join(images_dir, f"{config['path']}00{config['ext']}")
    if not os.path.exists(first_page_path):
        print(f"[IMAGE MISSING] ID: {book_id}, Path: {first_page_path}")

    # Check audio
    if config['audio'] == 'story.mp3':
        folder_path = config['path'][:config['path'].rfind('/') + 1]
        audio_path = os.path.join(images_dir, folder_path, 'story.mp3')
    else:
        audio_path = os.path.join(audio_dir, config['audio'])

    if not os.path.exists(audio_path):
        print(f"[AUDIO MISSING] ID: {book_id}, Path: {audio_path}")
